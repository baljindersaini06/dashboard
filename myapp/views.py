...
User = get_user_model()
from myapp.models import SiteConfiguration,SmtpConfiguration, Company, Employee
...
#     print(comp)
#     return render(request, 'myapp/company_list.html', {'comp':comp})

def companyview(request):
    test_all = Company.objects.all().values('company_name', 'description', 'company_website', 'company_address', 'company_location', 'company_pincode')
    test_all = json.dumps({"data": list(test_all)})
    data = {'test_data': test_all,}
    return render(request, 'myapp/company_list.html', data)
